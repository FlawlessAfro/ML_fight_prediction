...
@app.callback(
    [dash.dependencies.Output('probability-output', 'children'),
     ...
     dash.dependencies.Output('odds-output', 'children')],  # Add this line
...
def predict_victory(n_clicks, fighter1, fighter2):
    if n_clicks > 0:
        try:
            ...
            odds_message = "Odds:\n" + "\n".join(
                [f"{row['bookmaker']}: {row['odds_a']} vs {row['odds_b']}" for _, row in odds_info.iterrows()]
